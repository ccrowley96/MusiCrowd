...
router.post("/remove_song", async (req, res) => {
	...
	try {
		...
		return res
			.status(400)
			.send({ error: "Could not delete song from queue." });
	}
});
...
router.post("/vote", async (req, res) => {
	...
	try {
		await Room.findOneAndUpdate(
			{ party_code: party_code, "queue.song_id": song_id },
			{ $inc: { "queue.$.votes": incrementAmount } }
		);
	...
	try {
		...
		for (i = 0; i < room.queue.length; i++) {
			if (room.queue[i].votes > max) {
				...
				song_payload = room.queue[i].song_payload;
			}
		}

		await Room.findOneAndUpdate(
			{ party_code: party_code },
			{ $set: { currently_playing: song_payload } }
		);

		await Room.findOneAndUpdate(
			{ party_code: party_code },
			{ $pull: { queue: { song_id: song_payload.id } } }
		);
